...
export async function POST(request: NextRequest): Promise<NextResponse> {
    try {
        ...
        const { id, couponName, couponDescription, lastValidityDate, couponType, couponValue, minimumAmount } = reqBody;
        ...
        return NextResponse.json({
            success: true,
            message: "coupon has been edited successfully"
        }, 
        {
            status: 200
        })
        ...
        return NextResponse.json({
            success: false,
            message: "error occured while editing coupon"
        }, 
        {
            status: 500
        })
